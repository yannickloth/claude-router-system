...
class SemanticCache:
    ...
    def get(
        ...
        """
        Get cached result for a request (simple lookup by key or similarity).

        This is a convenience method that returns just the result value,
        not the full CachedResult object.

        Args:
            request: Request text to look up
            agent: Optional agent filter (if None, matches any agent)
            context_files: Optional context files for validation
        ...
        if cache_key in self.cache_index:
            ...
            # Verify agent matches if specified
            if agent is None or cached.agent_used == agent:
                # Check TTL
                age = datetime.now() - cached.timestamp
                if age <= timedelta(days=self.ttl_days):
                    cached.hit_count += 1
        ...
        # Fall back to similarity search if agent specified
        if agent is not None:
            similar = self.find_similar(request, agent, context_files)
            if similar:
    ...
    def clear(self) -> None:
        ...
        Removes all entries from the in-memory index and persists the empty state.
        """
...
def test_semantic_cache() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print("Test 1: Basic store and retrieve")
        cache = SemanticCache(cache_dir=tmpdir, similarity_threshold=0.85, ttl_days=30)

        cache.store(
            request="Find papers on mitochondrial dysfunction",
            agent="literature-integrator",
            result=["Smith2024", "Jones2023"],
            quota_cost=15
        )

        result = cache.get("Find papers on mitochondrial dysfunction", agent="literature-integrator")
        assert result == ["Smith2024", "Jones2023"], f"Expected cached result, got {result}"
        print("  OK")
        ...
        vec2 = [1.0, 0.0, 0.0]
        vec3 = [0.0, 1.0, 0.0]
        assert cache._cosine_similarity(vec1, vec2) == 1.0, "Identical vectors should have similarity 1.0"
        assert cache._cosine_similarity(vec1, vec3) == 0.0, "Orthogonal vectors should have similarity 0.0"
        print("  OK")

        # Test 4: TF-IDF embedding
        print("Test 4: TF-IDF embedding fallback")
        embedding = cache._compute_tfidf_embedding("test query for embedding")
        assert len(embedding) == 384, f"Embedding should be 384 dimensions, got {len(embedding)}"
        # Check unit vector normalization
        magnitude = math.sqrt(sum(x * x for x in embedding))
        assert abs(magnitude - 1.0) < 0.01, f"Embedding should be unit vector, magnitude = {magnitude}"
        print("  OK")
        ...
        # Test 6: Context hash
        print("Test 6: Context-aware invalidation")
        # Create test file
        test_file = tmpdir / "test_context.py"
        test_file.write_text("original content")

        hash1 = cache._compute_context_hash([str(test_file)])

        # Modify file
        test_file.write_text("modified content")
        hash2 = cache._compute_context_hash([str(test_file)])

        assert hash1 != hash2, "Context hash should change when file changes"
        print("  OK")

        # Test 7: Cache persistence
        print("Test 7: Cache persistence")
        cache.store(
            request="Persistence test query",
            agent="test-agent",
        ...
        cache2 = SemanticCache(cache_dir=tmpdir, similarity_threshold=0.85, ttl_days=30)
        result = cache2.get("Persistence test query", agent="test-agent")
        assert result == {"persistent": True}, "Cache should persist across instances"
        print("  OK")
        ...
        # Test 9: Store with context files
        print("Test 9: Context-dependent caching")
        context_file = tmpdir / "context.py"
        context_file.write_text("context content")

        cache2.store(
            request="Context dependent query",
            agent="test-agent",
        ...
        result = cache2.find_similar(
            request="Context dependent query",
            agent="test-agent",
            context_files=[str(context_file)]
        )
        assert result is not None, "Should find with same context"
        print("  OK")

        # Test 10: Different agent filtering
        print("Test 10: Agent filtering")
        cache2.store(
            request="Multi-agent query",
            agent="agent-a",
            result="result-a",
            quota_cost=5
        )

        result_a = cache2.get("Multi-agent query", agent="agent-a")
        result_b = cache2.get("Multi-agent query", agent="agent-b")
        assert result_a == "result-a", "Should find for agent-a"
        assert result_b is None, "Should not find for agent-b"
        print("  OK")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if command == "stats":
            ...
            # Legacy test command - redirect to comprehensive test
            test_semantic_cache()
            sys.exit(0)

        else:
            print(f"Unknown command: {command}")
            print("Usage: python semantic_cache.py [--test|test|stats|cleanup]")
            sys.exit(1)
    ...
    print()
    print("Run 'python semantic_cache.py --test' for comprehensive testing")
