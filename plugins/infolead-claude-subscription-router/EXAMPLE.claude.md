...
This is a template for configuring the Infolead Claude Subscription Router in your project.
...
router → project-specific agent (from .claude/agents/)
      OR
router → general agent (infolead-claude-subscription-router:haiku-general, infolead-claude-subscription-router:sonnet-general, infolead-claude-subscription-router:opus-general)
      OR
router → infolead-claude-subscription-router:router-escalation (if uncertain about routing)
```
...
When `router` is uncertain, it delegates to `infolead-claude-subscription-router:router-escalation` with the full request. `router-escalation` analyzes the edge case, makes the routing decision, and spawns the appropriate agent directly.
...
- `infolead-claude-subscription-router:haiku-general`: Mechanical operations with no judgment needed (pattern matching, simple transforms)
- `infolead-claude-subscription-router:sonnet-general`: Default for tasks requiring reasoning, analysis, or judgment calls
- `infolead-claude-subscription-router:opus-general`: Complex reasoning, mathematical proofs, detecting subtle logical flaws
...
| --- | --- | --- |
| `infolead-claude-subscription-router:router` | Sonnet | Entry point for all requests; interprets intent, assesses risk, routes to appropriate agent |
| `infolead-claude-subscription-router:router-escalation` | Opus | Handles edge cases when router is uncertain; makes decision and spawns agent directly |
| `infolead-claude-subscription-router:strategy-advisor` | Sonnet | Analyzes execution cost/benefit and recommends execution strategy (direct vs propose-review) |

### General Agents

| Agent | Model | Use when... |
| --- | --- | --- |
| `infolead-claude-subscription-router:haiku-general` | Haiku | Mechanical tasks, no judgment needed, explicit paths |
| `infolead-claude-subscription-router:sonnet-general` | Sonnet | Default for reasoning, analysis, judgment calls |
| `infolead-claude-subscription-router:opus-general` | Opus | Complex reasoning, proofs, high-stakes decisions |
...
| Disambiguation (rare) | Full namespace | `infolead-claude-subscription-router:haiku-general` |
...
⚠️ subagent_type: "infolead-claude-subscription-router:haiku-general"  # Only if disambiguation needed
```
...
```text
✅ subagent_type: "infolead-claude-subscription-router:haiku-general"              # Uses haiku (from agent definition)
✅ subagent_type: "infolead-claude-subscription-router:sonnet-general"             # Uses sonnet (from agent definition)
⚠️ subagent_type: "infolead-claude-subscription-router:haiku-general", model: "sonnet"  # Override - use only when intentional
```
