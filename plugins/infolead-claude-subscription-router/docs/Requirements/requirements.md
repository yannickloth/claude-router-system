...
**Requirement:** System MUST collect metrics for performance monitoring.
...
1. `agent_event` - Raw events from hooks (agent starts/stops)
2. `solution_metric` - Computed/aggregated solution metrics

**Metrics per solution:**

| Solution | Metric | Target |
|----------|--------|--------|
| haiku_routing | escalation_rate | 30-40% |
| work_coordination | completion_rate | 90-100% |
| domain_optimization | detection_accuracy | 95-100% |
| temporal_optimization | quota_utilization | 80-90% |
| deduplication | cache_hit_rate | 40-50% |
| probabilistic_routing | optimistic_success | 85-100% |
| state_continuity | save_success | 98-100% |
| context_ux | avg_response_time | 0-5s |

**CLI commands:**

```bash
python3 metrics_collector.py compute      # Compute all solution metrics
python3 metrics_collector.py efficiency   # Show routing efficiency vs baseline
python3 metrics_collector.py dashboard    # Live status dashboard
python3 metrics_collector.py report daily # Daily report
python3 metrics_collector.py report weekly # Weekly report
```

**Acceptance Criteria:**

- Agent events recorded automatically via SubagentStop hook
- Solution metrics computed from events and module state
- Efficiency calculation compares actual cost vs all-sonnet baseline
- Status assessment: on_target, warning, critical for each solution
